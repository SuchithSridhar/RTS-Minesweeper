...
MenuButton *createMenuButtonArray(int num_menu_button, char *titles[], int *screen_width, int *screen_height){
    MenuButton *menu_button_array = malloc(sizeof(MenuButton) * num_menu_button);
...
void initStartMenu(MenusBundle *menu_bundle, int *screen_width, int *screen_height){
    ...
    char *titles[] = {START_BUTTON_TITLE, SETTINGS_BUTTON_TITLE, STATS_BUTTON_TITLE, QUIT_BUTTON_TITLE};
    menu->menu_button_array = createMenuButtonArray(NUM_MENU_BUTTONS, titles, screen_width, screen_height);
...
void initStartGameSettings(MenusBundle *menu_bundle, int *screen_width, int *screen_height){
    ...
    start_settings->menu_button_array = createMenuButtonArray(NUM_SETTINGS_BUTTONS, titles, screen_width, screen_height);
...
void initSettings(MenusBundle *menu_bundle, int *screen_width, int *screen_height){
    ...
    settings->menu_button_array = createMenuButtonArray(NUM_SETTINGS_BUTTONS, titles, screen_width, screen_height);
...
void initStatistics(MenusBundle *menu_bundle, int *screen_width, int *screen_height){
    ...
    statistics->menu_button_array = createMenuButtonArray(NUM_STATS_BUTTONS, titles, screen_width, screen_height);
...
void setMenuBounds(StartMenu *menu, int *screen_width, int *screen_height){
    ...
    for(int i = 0; i < NUM_MENU_BUTTONS; i++){
        menu->menu_button_array[i].bounds = initMenuButtonBounds(*screen_width / 2.0f - button_width / 2.0f, i * (*screen_height / 2.0f - button_height / 2.0f) 
                                                                 / NUM_MENU_BUTTONS + *screen_height / 4.0f, button_width, button_height);
    }
}
...
void setStartGameBounds(StartSettings *start_settings, int *screen_width, int *screen_height){
    ...
    for(int i = 0; i < NUM_SETTINGS_BUTTONS; i++){
        if(i == 0){
            start_settings->menu_button_array[i].bounds = initMenuButtonBounds(position.x + button_width / 7.0f, (position.y + *screen_height / 1.5f) 
                                                                              - button_height - button_height / 7.0f, button_width, button_height);
        }
        else{
            start_settings->menu_button_array[i].bounds = initMenuButtonBounds((position.x + *screen_width / 2.0f) - button_width - button_width / 7.0f, 
                                                                               (position.y + *screen_height / 1.5f) - button_height - button_height / 7.0f, 
...
void setSettingsBounds(Settings *settings, int *screen_width, int *screen_height){
    ...
    for(int i = 0; i < NUM_SETTINGS_BUTTONS; i++){
        if(i == 0){
            settings->menu_button_array[i].bounds = initMenuButtonBounds(position.x + button_width / 7.0f, (position.y + *screen_height / 1.5f) 
                                                                        - button_height - button_height / 7.0f, button_width, button_height);
        }
        else{
            settings->menu_button_array[i].bounds = initMenuButtonBounds((position.x + *screen_width / 2.0f) - button_width - button_width / 7.0f, 
                                                                         (position.y + *screen_height / 1.5f) - button_height - button_height / 7.0f, 
                                                                         button_width, button_height);
        }
...
void setStatsBounds(Statistics *stats, int *screen_width, int *screen_height){
    ...
    for(int i = 0; i < NUM_STATS_BUTTONS; i++){
        stats->menu_button_array[i].bounds = initMenuButtonBounds(position.x + *screen_width / 4.0f - button_width / 2.0f, 
                                                                 (position.y + *screen_height / 1.5f) - button_height -button_height / 7.0f, 
                                                                 button_width, button_height);
    }
}
...
void drawMenuButton(MenuButton *menu_button){
    ...
    text_position.x = menu_button->bounds.x + (menu_button->button_width / 2.0f) - ((float)MeasureText(menu_button->text, FONT_SIZE_BUT)) / 2.0f;
    text_position.y = menu_button->bounds.y + (menu_button->button_height / 2.0f);
    DrawRectangleRec(menu_button->bounds, BLACK);
    DrawText(menu_button->text, text_position.x, text_position.y, FONT_SIZE_BUT, RAYWHITE);
}
...
void drawStartMenu(StartMenu *menu, int *screen_width, int *screen_height){
    ...
    title_position.x = *screen_width / 2.0f - ((float)MeasureText(menu->title, FONT_SIZE_TITLE)) / 2;
...
void mouseHoverButton(MenuButton *button){
    ...
    Vector2 text_position;
    text_position.x = button->bounds.x + (button->bounds.width / 2.0f) - ((float)MeasureText(button->text, FONT_SIZE_BUT)) / 2.0f;
...
void handleStateClicks(MenusBundle *menu_bundle, Vector2 *mp, int *state){
    *mp = GetMousePosition();
    switch (*state){
    case (STATE_START):
        if (IsMouseButtonReleased(MOUSE_BUTTON_LEFT) && menu_bundle->menu->button_mouse_over > NO_HOVER)
    ...
    case (STATE_SETTINGS):
        ...
        break;
    case (STATE_STATISTICS):
        if (IsMouseButtonReleased(MOUSE_BUTTON_LEFT) && menu_bundle->statistics->button_mouse_over > NO_HOVER)
